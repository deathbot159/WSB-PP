...
class StudentsDB:
    def __init__(self, menu: consolemenu.console_menu.ConsoleMenu):
        self.students = []
        self.uniqueAlbumNr: list[int] = []
        self.provided = {}
        self.menuRef = menu
        self.menuRef.subtitle = f"Aktualna liczba studentów w bazie: {len(self.students)}"
    ...
    def addStudent(self):
        ...
        if "albumnr" not in self.provided:
            try:
                albumnr = int(input("Podaj numer albumu: "))
                f = False
                if albumnr in self.uniqueAlbumNr:
                    print(f"Nr albumu {albumnr} ma już przypisanego studenta!")
                    self.addStudent()
                    return
                self.provided["albumnr"] = albumnr
            except ValueError:
                print("Nie poprawny typ podanej wartości.")
                self.addStudent()
                return
        ...
        self.students.append(
            StudentData(len(self.students) + 1, self.provided["name"], self.provided["surname"],
                        ...
                        self.provided["grade"]))
        self.uniqueAlbumNr.append(self.provided['albumnr'])
        self.menuRef.subtitle = f"Aktualna liczba studentów w bazie: {len(self.students)}\n" \
    ...
    def removeStudent(self, tablePrinted: bool = False):
        ...
        try:
            ...
            if len(searchArr) == 0:
                ...
                return
            self.menuRef.subtitle = f"Aktualna liczba studentów w bazie: {len(self.students) - 1}\nSukces: Usunięto studenta {searchArr[0].name} {searchArr[0].surname} nr alb.: {searchArr[0].albumnr}."
            self.students.remove(searchArr[0])
            self.uniqueAlbumNr.remove(searchArr[0].albumnr)
            Console.clear()
            return
    ...
    def loadFromFile(self):
        ...
        soft_problems = 0
        crit_problems = 0
        subtitleText = f"Aktualna liczba studentów w bazie: %arrLen%\nSukces: Dodano studentów z pliku. Liczba " \
        ...
        for data in dataReader:
            ...
            albumnrexists = int(data[3]) in self.uniqueAlbumNr
            if albumnrexists:
            ...
            if not idexists and not albumnrexists:
                self.students.append(StudentData(data[0], data[1], data[2], data[3], data[4]))
